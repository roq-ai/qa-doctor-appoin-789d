...
import { getReceptionists } from 'apiSdk/receptionists';
import { ReceptionistInterface } from 'interfaces/receptionist';

import { ReceptionistListPage as ShowCaseList } from 'pages/receptionists';
...
function DashboardPage() {
  ...
  const receptionistStat = useSWR<PaginatedInterface<ReceptionistInterface>>(
    () => `/receptionists`,
    () => getReceptionists({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Receptionists',
      result: receptionistStat?.data?.totalCount,
      isLoading: receptionistStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Receptionists
                </Text>
                <NextLink href={`receptionists`} passHref legacyBehavior>
                  <Button
